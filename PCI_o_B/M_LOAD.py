        ...
        for i in range(len(self.d)):
            if len(self.F_N_unload[i]) != 0:
                ...
                if plot == True:
                    plt.figure()
                    ax = plt.axes()
                    ax.plot(new_d,load(new_d),marker='o',linestyle='',color='green',label='load')
                    ax.plot(new_d,unload(new_d),marker='s',linestyle='',color='red',label='unload')
                    ax.fill_between(new_d,load(new_d),unload(new_d), step="pre", alpha=0.4,color='blue')
